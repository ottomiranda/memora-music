...
export function createTranslatedSchema<T extends z.ZodRawShape>(
  schemaDefinition: (t: (key: string) => string) => T
) {
  return (namespace: string = 'validation') => {
...
export function getTranslatedErrorMap(namespace: string = 'validation') {
  const { t } = useTranslation(namespace);
...
export function useTranslatedSchemas(namespace: string = 'validations') {
  const { t } = useTranslation(namespace);
  ...
  return {
    ...
    getAuthSchema: (isLogin: boolean) => isLogin ? loginSchema : signupSchema,
  };
}

// Função para extrair mensagens de erro traduzidas
export function getTranslatedValidationErrors(
  error: z.ZodError,
  namespace: string = 'validations'
