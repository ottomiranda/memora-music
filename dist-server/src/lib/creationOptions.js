...
export const getOccasionCategories = (t) => OCCASION_CATEGORY_CONFIG.map(({ key, labelKey, itemKeys }) => ({
    key,
    label: t(labelKey),
    items: itemKeys.map((itemKey) => ({ key: itemKey, label: t(itemKey) })),
}));
export const getRelationshipCategories = (t) => RELATIONSHIP_CATEGORY_CONFIG.map(({ key, labelKey, itemKeys }) => ({
    key,
    label: t(labelKey),
    items: itemKeys.map((itemKey) => ({ key: itemKey, label: t(itemKey) })),
}));
export const getEmotions = (t) => EMOTION_KEYS.map((key) => ({ key, label: t(key) }));
export const getVocalPreferences = (t) => VOCAL_KEYS.map((key) => ({ key, label: t(key) }));
const optionConfig = {
...
export const normalizeOptionValue = (type, value) => {
    if (!value)
        return value;
...
export const translateOptionValue = (type, value, t) => {
    if (!value)
        return '';
...
export const getLocalizedOptionValue = (type, value, language) => {
    if (!value)
        return '';
    const normalized = normalizeOptionValue(type, value);
    const { keys } = optionConfig[type];
