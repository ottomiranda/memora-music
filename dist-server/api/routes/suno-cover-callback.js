...
router.post('/', async (req, res) => {
    try {
        ...
        const coverTaskId = payload?.data?.taskId || payload?.taskId || payload?.data?.id;
        const imageUrl = payload?.data?.imageUrl || payload?.data?.image_url || payload?.imageUrl || payload?.image_url || payload?.data?.url;
        ...
        if (imageUrl) {
            ...
            if (originalTaskId) {
                const { data, error } = await client
                    ...
                    .limit(1);
                if (error)
                    console.error('[SUNO_COVER_CALLBACK] Erro ao atualizar por task_id:', error);
                else
                    console.log('[SUNO_COVER_CALLBACK] Capa atualizada para música:', data?.[0]?.id);
            }
        }
        res.status(200).json({ ok: true });
    }
...
//# sourceMappingURL=suno-cover-callback.js.map