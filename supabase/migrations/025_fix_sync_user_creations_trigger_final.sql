-- Correção final do trigger sync_user_creations para usar estrutura real das tabelas
-- Esta migration corrige o problema onde o trigger tentava usar client_ip que não existe na tabela songs
...
-- Criar nova função corrigida para estrutura real
...
BEGIN
  ...
  -- Nota: Removido client_ip pois não existe na tabela songs
  ...
  VALUES (
    computed_device_id,
    1,
    1, -- Incrementar freesongsused também
  ...
  DO UPDATE SET 
    ...
    freesongsused = user_creations.freesongsused + 1,
    updated_at = NOW(),
...
COMMENT ON FUNCTION public.sync_user_creations() IS 'Função trigger corrigida para sincronizar contagem de criações usando estrutura real das tabelas';
COMMENT ON TRIGGER trigger_sync_user_creations ON public.songs IS 'Trigger corrigido que atualiza user_creations quando uma nova música é criada';
