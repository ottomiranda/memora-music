...
      // MODIFICAÇÃO: Velocidade reduzida em 50% (5000 -> 10000)
      this.time = Date.now() / 10000;
      this.graphics.clear();
      for (var i = 0; i < this.MAX_LINES; i++) {
...
      // MODIFICAÇÃO: Velocidade reduzida em 50% (4000 -> 8000)
      var dx = w * 1 / 3 + w / 10 * Math.sin(Date.now() / 8000);
      var dy = h * 1 / 3;
      var size = w / 2;
      // もやっとした円
...
      // MODIFICAÇÃO: Velocidade reduzida em 50% (10000 -> 20000)
      var dx = w * 3 / 4 + w / 15 * Math.cos(Date.now() / 20000);
      var dy = h * 2 / 3;
      var size = w / 3;
      // もやっとした円
...
      // MODIFICAÇÃO: Velocidade reduzida em 50% (400 -> 800)
      var hue = Math.sin(-1 * Date.now() / 800 * Math.PI / 180) * 45 + 200;
      // 背景
...
     */
    function Particle() {
      // --- MODIFICAÇÃO INÍCIO: Substituído construtor de Shape por Text ---
      
      // Lista de ícones para usar como partículas
      var icons = ['❤️', '♪', '♫', '♩'];
      var icon = icons[Math.floor(Math.random() * icons.length)];

      // MAX_SIZE precisa ser definido antes de ser usado
      this.MAX_SIZE = 128;
      var size = Math.random() * Math.random() * Math.random() * Math.random() * this.MAX_SIZE + 2;
      this.size = size;

      // Gerar cor vibrante aleatória
      var hue = Math.random() * 360; // Cor aleatória
      var lightness = 50 + Math.random() * 30; // Brilho (50-80)
      var colorHsl = createjs.Graphics.getHSL(hue, 100, lightness);

      // Definir tamanho da fonte
      var fontSize = this.size * 2; // O tamanho do emoji pode precisar de ajuste
      
      // Chamar o construtor da classe pai (createjs.Text)
      _super.call(this, icon, fontSize + "px Arial", colorHsl);

      // Centralizar o texto (para que escale e rotacione a partir do centro)
      this.textAlign = "center";
      this.textBaseline = "middle";

      // --- CÓDIGO ANTIGO REMOVIDO (todo o bloco this.graphics.drawCircle) ---
      
      // Propriedades mantidas do original
      this.compositeOperation = "lighter";
      this.mouseEnabled = false;
      
      var padding = 2;
      // Ajustar o cache para o tamanho da fonte
      var cacheBoxSize = fontSize + padding * 2;
      this.cache(-fontSize / 2 - padding, -fontSize / 2 - padding, cacheBoxSize, cacheBoxSize);
      
      this._destroy = true;
      
      // --- MODIFICAÇÃO FIM ---
...
      this.x = emitX;
      this.y = emitY;
      // MODIFICAÇÃO: Velocidade inicial reduzida em 50% (2.0 -> 1.0)
      this.vx = (Math.random() - 0.5) * 1.0;
      this.vy = (Math.random() - 0.5) * 1.0;
      // MODIFICAÇÃO: Tempo de vida duplicado (400+40 -> 800+80)
      this.life = Math.random() * Math.random() * 800 + 80;
...
      // 重力計算
      // MODIFICAÇÃO: Gravidade reduzida em 50% (0.03 -> 0.015)
      this.vy -= 0.015;
      // 摩擦計算
      this.vx *= 0.99;
      this.vy *= 0.99;
...
    };
    return Particle;
  // MODIFICAÇÃO: Classe base alterada de Shape para Text
  })(createjs.Text);
})(project || (project = {}));
///<reference path="../libs/easeljs/easeljs.d.ts" />
