...
function createEnglishLyricsPrompt(data: z.infer<typeof generatePreviewSchema>): string {
...
function createEnglishLyricsAndTitlePrompt(data: any): string {
  const language = data.language || 'en-US';
...
function parseAIResponse(response: string): { title?: string; lyrics: string } {
...
router.post('/test-english', upload.none(), async (req: Request, res: Response) => {
  try {
