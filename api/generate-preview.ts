...
// Interface para definir o tipo das tarefas de música
...
// Armazenamento global para tarefas (em produção, usar Redis ou DB)
declare global {
  ...
  try {
    ...
    while (attempts < maxAttempts && task.status === 'PROCESSING') {
      ...
      try {
        ...
        console.log(`❌ [${taskId}] Erro na tentativa ${attempts}:`, (error as Error).message);
...
export default async function handler(req: Request, res: Response) {
  ...
  try {
    ...
    if (errorObj?.response) {
      const response = errorObj.response as { data?: unknown; status?: number; headers?: Record<string, string> };
